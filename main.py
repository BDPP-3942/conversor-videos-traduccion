...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config", type=Path, default=BASE_DIR / "config" / "app.toml", help="Path to the TOML configuration file"
    )
    sub = parser.add_subparsers(dest="command", required=True)
    run = sub.add_parser(
        "run",
        help="Run one unattended processing batch using the saved active profile",
        description="Process source videos through the complete STT, translation and optional TTS pipeline.",
    )
    run.add_argument(
        "--scheduled",
        action="store_true",
        help="Run in unattended scheduled-task mode; never opens a browser or asks for interactive input",
    )
    run.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate runtime/provider readiness and report effective settings without processing files",
    )
    run.add_argument(
        "--provider",
        choices=["local", "google_drive", "gdrive", "rclone"],
        default=None,
        help="Storage provider override; choices are local, google_drive/gdrive, or rclone. "
        "Default: active configured provider",
    )
    run.add_argument(
        "--source",
        default=None,
    ...
    run.add_argument(
        "--no-retain-sources",
        action="store_true",
        help="For local processing, disable retention of source files after normal processing. "
        "Not applicable to regeneration",
    )
    run.add_argument(
        "--no-resume",
        action="store_true",
        help="Disable normal resume/reuse behavior for this run. Regeneration already forces reprocessing "
        "and does not accept this flag",
    )
    run.add_argument(
        "--no-name-migration",
        action="store_true",
    ...
    run.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum requested video workers; 0 selects AUTO, 1 keeps one worker, "
        "and positive values are clamped to the safe hardware limit",
    )
    run.add_argument(
        "--translation-batch-size",
        type=int,
        default=None,
        help="Translation request batch size override. Values below 1 are normalized to the minimum valid size; "
    ...
    webm_group.add_argument(
        ...
        dest="generate_webm",
        action="store_true",
        help="Force generation of the secondary WebM output",
    )
    webm_group.add_argument(
    ...
    duplicates = sub.add_parser(
        "duplicates",
        help="Inspect and manage duplicate local output folders",
        description="Scan, analyze and optionally delete duplicate local output folders "
        "using the persisted deletion plan.",
    )
    duplicates.add_argument(
        "--target",
        type=Path,
        default=BASE_DIR / "storage" / "output",
        help="Local output directory to inspect. Default: storage/output",
    )
    ...
    delete_duplicates = duplicate_sub.add_parser(
        "delete", help="Delete only duplicates present in the persisted analysis plan"
    )
    ...
    auth = sub.add_parser(
        "auth",
        help="One-time interactive authentication",
        description="Authorize a provider for a named persistent profile.",
    )
    auth.add_argument(
        "provider", choices=["google"], help="Authentication provider. Currently only google is supported"
    )
    auth.add_argument("--profile", default="default", help="Provider profile name. Default: default")
    provider = sub.add_parser(
        "provider",
        help="Configure and switch persistent provider profiles",
        description="Manage storage provider profiles and the active runtime selection.",
    )
    ...
    verify = provider_sub.add_parser(
        "verify",
        help="Run a read-only cloud credential check",
        description="Verify access without changing provider configuration.",
    )
    ...
    verify.add_argument(
        "--location", default="", help="rclone folder used for the read-only health check. Default: empty"
    )
    ...
    setup_google.add_argument(
        "--archive-folder-id", default="", help="Optional Google Drive archive folder ID. Default: empty"
    )
    ...
    auth_rclone.add_argument(
        "--non-interactive",
        action="store_true",
        help="Create the remote without launching interactive rclone configuration",
    )
    ...
    reprocess = sub.add_parser(
        ...
        description="Repair or regenerate subtitle stages in existing results "
        "while leaving the audiovisual media untouched.",
    )
    ...
    reprocess.add_argument(
        "--output-folder",
        default=None,
        help="Existing output folder to process. Default: select by other selectors or --all",
    )
    reprocess.add_argument(
        "--all",
        dest="reprocess_all",
        action="store_true",
        help="Process all eligible output folders; cannot be combined with other selectors",
    )
    reprocess.add_argument(
        "--video",
        dest="video_name",
        default=None,
        help="Video/source name selector inside the output set. Default: none",
    )
    reprocess.add_argument(
        "--source", default=None, help="Source URI or source selector used by the reprocessor. Default: none"
    )
    reprocess.add_argument(
        "--scheduled",
        action="store_true",
        help="Use saved provider configuration without interactive provider overrides",
    )
    reprocess.add_argument(
        "--provider",
        choices=["local", "google_drive", "gdrive", "rclone"],
        default=None,
        help="Storage provider override. Default: active configured provider",
    )
    reprocess.add_argument(
        "--target", default=None, help="Target storage URI override. Default: active configured target"
    )
    sub.add_parser(
        "prefetch-whisper",
        help="Download/initialize the automatically selected Whisper model",
        description="Initialize the configured Whisper/STT model "
        "so subsequent processing can use it without first downloading it.",
    )
    sub.add_parser(
        "doctor",
        help="Check interactive and unattended runtime readiness",
        description="Check configuration, Python, FFmpeg, Whisper/runtime and provider readiness.",
    )
    sub.add_parser(
        "init",
        help="Create runtime directories",
        description="Create the runtime directories required by the pipeline.",
    )
    return parser
