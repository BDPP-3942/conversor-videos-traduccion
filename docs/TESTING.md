...
Naming tests cover both layers of the contract: inference of the logical ZIP/course/resource name and the final physical filesystem representation. The latter is tested for separator normalization, punctuation/control handling, accent transliteration, Windows reserved names and the `x` scope separator. Reference-tree examples are treated as functional fixtures, not as runtime data.
