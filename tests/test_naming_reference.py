...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            "estas-son-promocinales-son-6_CHINNA_EN_Si_Zheng_Tui",
        ),
    ),
)
...
def test_physical_naming_uses_underscore_scope_separator(tmp_path: Path) -> None:
    physical = fit_output_stem("19x2-POSTURAS (FIJAS)", tmp_path)
    assert physical == "19_2_POSTURAS_FIJAS"
    assert "x" not in physical
