...
def main() -> int:
    ...
    if args.resource in {"cuda", "all"}:
        if args.action == "status":
            ...
            print(json.dumps({"resource": "cuda", "compatible": status.compatible, "nvidia_gpu": status.nvidia_gpu, "driver": status.driver_version, "driver_cuda_max": status.driver_cuda_max, "toolkit": status.toolkit_version, "toolkit_path": status.toolkit_path, "cuBLAS": status.cublas_version, "cuDNN": status.cudnn_version, "faster_whisper": status.faster_whisper_version, "ctranslate2": status.ctranslate2_version, "managed_path": str(MANAGED_DIR), "reason": status.reason}, ensure_ascii=False, indent=2))
        else:
