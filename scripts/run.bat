...
IF NOT EXIST "%VENV_DIR%\Scripts\activate.bat" (
    echo [ERROR] No existe el venv. Ejecuta primero scripts\setup_env.bat
    exit /b 1
)
...
python main.py --source "%DRIVE_SOURCE%" --target "%DRIVE_TARGET%" --mode %EXEC_MODE% --source-lang "es" --target-lang "en" --whisper-model "small"
