...
from src.translation_providers import TranslationProvider, TranslationQuotaError, build_translation_provider
    ...
    def _translate_batch(self, texts: list[str], indexes: list[int]) -> tuple[list[str], dict[int, str], dict[int, list[str]]]:
        ...
        for position, name in enumerate(names):
            ...
            if next_unresolved and position < len(names) - 1:
                logger.info("Provider '%s' left %d unresolved segment(s); falling back", name, len(next_unresolved))
        ...
        for attempt in range(1, BATCH_MAX_ATTEMPTS + 1):
            ...
            try:
                ...
                if not isinstance(result, list) or len(result) != len(texts):
                    raise RuntimeError(f"Translator returned {len(result) if isinstance(result, list) else 'invalid'} items for {len(texts)} inputs")
