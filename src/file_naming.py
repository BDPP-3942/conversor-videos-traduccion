...
class FileNameFormatter:
    ...
    LESSON_TEXT_PATTERNS = (re.compile(r"\b(?:lecci[oó]n|lesson|cap[ií]tulo|chapter|clase|tema|unidad)\s*[:\-–—.]?\s*([^|/\\]+)", re.IGNORECASE),)
    NOISE_PATTERNS = (
    ...
    GENERIC_TOKENS = {"mp4", "wmv", "video", "videos", "audio", "media", "file", "files", "archivo", "archivos", "download", "downloads", "descarga", "descargas", "compressed", "compression", "archive", "archivo_comprimido", "zip", "rar", "7z"}
    FILENAME_ARTIFACT_PATTERN = re.compile(r"(?:^|[_\- .])(?:\d{8}t\d{4,6}z(?:[-_]\d+[-_]\d+)?)(?:[_\- .]|$)", re.IGNORECASE)
    ...
    def _remove_number(cls, value: str, number: int) -> str:
        for pattern in (re.compile(rf"(?<!\d){number:02d}(?!\d)"), re.compile(rf"(?<!\d){number}(?!\d)")):
