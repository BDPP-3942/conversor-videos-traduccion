...
def resolve_initial_prompt(value: str, base_dir: Path | None = None) -> tuple[str, str]:
    ...
    Existing literal prompts remain fully compatible. A supported file path is
    loaded as text, Markdown, CSV cells, or DOCX paragraph text. An omitted
    value may use the conventional ``palabras_contexto.<ext>`` file when one
    exists next to the configured project context.
    """
    ...
    if not raw:
        ...
        if path is None:
            if Path(raw).suffix.lower() in SUPPORTED_EXTENSIONS or Path(raw).name.lower().startswith("palabras_contexto."):
