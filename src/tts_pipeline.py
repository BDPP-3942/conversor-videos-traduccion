...
class KokoroONNXProvider:
    ...
    def _load(self):
        ...
        try:
            ...
            raise TTSProviderError("Kokoro TTS is not installed. Install the optional TTS dependencies before enabling TTS.") from exc
    ...
    def synthesize(self, text: str, *, language: str, voice: str, speed: float) -> tuple[object, int]:
        engine = self._load()
        try:
            samples, sample_rate = engine.create(text, voice=voice, speed=speed, lang=_kokoro_language(language))
...
def create_tts_provider(settings: AppSettings) -> TTSProvider:
    ...
    return KokoroONNXProvider(resolve_project_path(settings.tts_model_path), resolve_project_path(settings.tts_voices_path))


def generate_tts_media(video_path: Path, translated_vtt_path: Path, output_dir: Path, output_stem: str, settings: AppSettings, *, webm_video_path: Path | None = None, provider: TTSProvider | None = None) -> TTSResult:
...
def _render_timeline(cues: list[TTSCue], provider: TTSProvider, settings: AppSettings, output: Path) -> int:
    import numpy as np
    ...
    for cue_index, cue in enumerate(cues, 1):
        ...
        samples, generated_rate = provider.synthesize(cue.text, language=settings.target_lang, voice=settings.tts_voice, speed=speed)
        ...
        if len(audio) > target_samples:
            if len(audio) > target_samples * (1.0 + settings.tts_duration_tolerance):
                raise TTSProviderError(f"Cue {cue_index} cannot fit in {cue.duration:.3f}s at max TTS speed {settings.tts_max_speed:.2f}")
...
def _mux_video(source: Path, audio: Path, output: Path, settings: AppSettings, *, webm: bool) -> None:
    ...
    command = [str(ffmpeg), "-hide_banner", "-loglevel", "error", "-y", "-i", str(source.resolve()), "-i", str(audio.resolve()), "-map", "0:v:0", "-map", "1:a:0"]
    if webm and source.suffix.lower() == ".webm":
        command += ["-c:v", "copy", "-c:a", "libopus", "-b:a", settings.tts_webm_audio_bitrate]
    elif webm:
        command += ["-c:v", "libvpx-vp9", "-crf", "32", "-b:v", "0", "-c:a", "libopus", "-b:a", settings.tts_webm_audio_bitrate]
    else:
        command += ["-c:v", "copy", "-c:a", "aac", "-b:a", settings.tts_audio_bitrate, "-movflags", "+faststart"]
    command.append(str(output.resolve()))
    _run_ffmpeg(command, settings.ffmpeg_timeout_seconds)


def _validate_media(path: Path, settings: AppSettings, *, require_video: bool, require_audio: bool, expected_audio: str | None = None) -> None:
    if not path.is_file() or path.stat().st_size <= 0:
        raise RuntimeError(f"Media artifact is missing or empty: {path}")
    ffmpeg = _resolve_ffmpeg(settings)
    command = [str(ffmpeg), "-hide_banner", "-i", str(path.resolve())]
    try:
        result = subprocess.run(command, capture_output=True, text=True, timeout=60, check=False, shell=False)  # noqa: S603 -- trusted absolute executable, argv list, shell disabled
...
def _run_ffmpeg(command: list[str], timeout: int) -> None:
    ...
    try:
        result = subprocess.run(command, capture_output=True, text=True, timeout=max(30, timeout), check=False, shell=False)  # noqa: S603 -- trusted executable/argv generated internally, shell disabled
...
def _kokoro_language(language: str) -> str:
    ...
    return {"en": "en-us", "en-us": "en-us", "en-gb": "en-gb", "es": "es", "es-es": "es", "fr": "fr-fr", "it": "it", "pt": "pt-br", "pt-br": "pt-br", "ja": "ja", "zh": "zh", "hi": "hi"}.get(normalized, normalized)
