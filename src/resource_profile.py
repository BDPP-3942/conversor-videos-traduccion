...
def safe_parallelism(settings: AppSettings, hardware: HardwareInfo | None = None) -> int:
    ...
    if device == "auto":
        device = "cuda" if hw.gpu.usable_for_whisper and hw.gpu.vram_free_gb >= 3.0 else "cpu"
    ...
    effective = min(requested, by_cpu, by_ram)
...
def build_profile(settings: AppSettings, hardware: HardwareInfo | None = None) -> ResourceProfile:
    ...
    if model == "auto":
        model = "medium" if (device == "cuda" and hw.gpu.vram_free_gb >= 8) or (memory >= 16 and cpus >= 6) else "small"
    ...
    return ResourceProfile(name, cpus, hw.physical_cpus, memory, available, model, device, compute, threads, effective_parallel, settings.translation_batch_size if settings.translation_batch_size > 0 else batch, hw.gpu.device_index, hw.gpu.vram_free_gb, hw.disk_free_gb)
