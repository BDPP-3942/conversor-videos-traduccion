...
class OutputPostProcessor:
    ...
    def process_folder(self, folder: str) -> dict[str, Any]:
        ...
        original_folder = next(
            (item for item in self.storage.list_children(folder) if item.is_directory and item.name == self.settings.original_transcript_subdir),
            None,
        )
        original = None
        if original_folder:
            original = self._pick_vtt(self.storage.list_children(original_folder.id), "_original")
        ...
        if video is None:
            ...
            result["reason"] = "No source video found in output folder"
            return result
        ...
        with tempfile.TemporaryDirectory(prefix="output-recovery-", dir=work) as tmp:
            ...
            if self.settings.tts_enabled:
                ...
                tts_exists = self.storage.file_exists(folder, tts_mp4_name)
                if tts_exists and not result["vtt_repaired"]:
    ...
    def _replace_vtt(self, current: StorageFile | None, parent: str, replacement: Path) -> None:
        ...
        if current:
            ...
            backup_name = f"{current.name}.bak.{datetime_utc_stamp()}"
            self.storage.upload_file(old, parent, "text/vtt")
            # Preserve the original content under a collision-safe history name.
            backup_local = replacement.parent / backup_name
            backup_local.write_bytes(old.read_bytes())
            self.storage.upload_file(backup_local, parent, "text/vtt")
