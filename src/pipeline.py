...
class MediaPipeline:
    ...
    def run(self, source: str, target: str) -> dict[str, Any]:
        ...
        for zip_file in zips:
            ...
            if result["status"] == "success":
                self.storage.finalize_source(zip_file, "success")

        failed = sum(item["status"] == "error" for item in results)
        partial = sum(item["status"] == "partial" for item in results)
        status = "error" if failed else "partial" if partial else "success"
        return {
            "status": status,
            "zips_found": len(zips),
    ...
    def _process_zip(self, zip_file: StorageFile, target: str) -> dict[str, Any]:
        work_root = local_storage_paths()["work"] / Path(zip_file.name).stem
        if work_root.exists():
        ...
        for source_path in extraction.media:
            try:
                processed.append(
                    self._process_media(source_path, extract_root, work_root, target, used_stems)
                )
            except Exception as exc:
        ...
        manifest = local_storage_paths()["output"] / f"manifest_{Path(zip_file.name).stem}.json"
    ...
    def _process_media(
        ...
        original_dir = local_storage_paths()["original_transcriptions"]
        original_path = original_dir / f"{stem}_original.vtt"
        ...
        output_dir = local_storage_paths()["output"]
        for local_path, mime in (
            (artifacts.mp4_path, "video/mp4"),
            (artifacts.mp3_path, "audio/mpeg"),
            (translated_path, "text/vtt"),
        ):
            self.storage.upload_file(local_path, target, mime)

        original_target = self._original_target(target)
    ...
    def _original_target(self, target: str) -> str:
        ...
        if location.scheme == "local":
            parent = resolve_project_path(location.value)
            return str(parent / self.settings.original_transcript_subdir)
        if location.scheme == "gdrive":
            return self.storage.ensure_folder(
                location.value, self.settings.original_transcript_subdir
            )
        if location.scheme == "rclone":
            return f"{location.value.rstrip('/')}/{self.settings.original_transcript_subdir}"
    ...
    def cleanup(self) -> None:
        if self.settings.provider.lower() == "local":
            return
        # Work data stays available until the batch result is written for diagnostics.
