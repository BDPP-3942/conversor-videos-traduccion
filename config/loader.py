...
def load_settings(config_path: Path | None = None) -> AppSettings:
    ...
    if not path.is_file():
        ...
        settings = AppSettings(
            ...
            local_translation_model_dir=_runtime_path(
            ...
            tts_model_path=_runtime_path(str(tts.get("model_path", "tools/tts/kokoro-v1.0.onnx"))),
            tts_voices_path=_runtime_path(str(tts.get("voices_path", "tools/tts/voices-v1.0.bin"))),
            ...
            google_credentials_file=_runtime_path(
                str(google.get("credentials_file", "secrets/providers/google/default/credentials.json"))
            ),
            google_token_file=_runtime_path(str(google.get("token_file", "secrets/providers/google/default/token.json"))),
            rclone_config_file=_runtime_path(str(rclone.get("config_file", "secrets/rclone/rclone.conf"))),
            rclone_binary_file=_runtime_path(str(rclone.get("binary_file", "tools/rclone/rclone"))),
            rclone_remote=str(rclone.get("remote", "remote_drive")),
            provider_profile_dir=_runtime_path(str(providers.get("profile_dir", "secrets/providers"))),
            run_lock_file=_runtime_path(str(runtime_cfg.get("run_lock_file", "storage/state/run.lock"))),
