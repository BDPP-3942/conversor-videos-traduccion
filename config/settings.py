...
class AppSettings:
    ...
    def from_environment(cls) -> AppSettings:
        ...
        return cls(
            provider=os.getenv("STORAGE_PROVIDER", cls.provider), source=os.getenv("SOURCE_URI", cls.source),
            target=os.getenv("TARGET_URI", cls.target), source_lang=os.getenv("SOURCE_LANG", cls.source_lang),
            target_lang=os.getenv("TARGET_LANG", cls.target_lang), log_level=os.getenv("LOG_LEVEL", cls.log_level).upper(),
            ...
            whisper_repetition_threshold=float(os.getenv("WHISPER_REPETITION_THRESHOLD", cls.whisper_repetition_threshold)),
            whisper_compression_ratio_threshold=float(os.getenv("WHISPER_COMPRESSION_RATIO_THRESHOLD", cls.whisper_compression_ratio_threshold)),
            whisper_log_prob_threshold=float(os.getenv("WHISPER_LOG_PROB_THRESHOLD", cls.whisper_log_prob_threshold)),
            whisper_no_speech_threshold=float(os.getenv("WHISPER_NO_SPEECH_THRESHOLD", cls.whisper_no_speech_threshold)),
            whisper_min_repetition_words=int(os.getenv("WHISPER_MIN_REPETITION_WORDS", cls.whisper_min_repetition_words)),
            ...
            max_zip_depth=int(os.getenv("MAX_ZIP_DEPTH", cls.max_zip_depth)), max_extracted_files=int(os.getenv("MAX_EXTRACTED_FILES", cls.max_extracted_files)),
            max_extracted_size_gb=float(os.getenv("MAX_EXTRACTED_SIZE_GB", cls.max_extracted_size_gb)), ffmpeg_bin=os.getenv("FFMPEG_BIN", cls.ffmpeg_bin),
            ffmpeg_preset=os.getenv("FFMPEG_PRESET", cls.ffmpeg_preset), ffmpeg_crf=int(os.getenv("FFMPEG_CRF", cls.ffmpeg_crf)),
            ffmpeg_audio_bitrate=os.getenv("FFMPEG_AUDIO_BITRATE", cls.ffmpeg_audio_bitrate), generate_webm=os.getenv("GENERATE_WEBM", "true").lower() == "true",
            secondary_video_extension=os.getenv("SECONDARY_VIDEO_EXTENSION", cls.secondary_video_extension), secondary_video_codec=os.getenv("SECONDARY_VIDEO_CODEC", cls.secondary_video_codec),
            secondary_video_crf=int(os.getenv("SECONDARY_VIDEO_CRF", cls.secondary_video_crf)), secondary_video_max_width=int(os.getenv("SECONDARY_VIDEO_MAX_WIDTH", cls.secondary_video_max_width)),
            secondary_video_fps=int(os.getenv("SECONDARY_VIDEO_FPS", cls.secondary_video_fps)), secondary_video_audio_codec=os.getenv("SECONDARY_VIDEO_AUDIO_CODEC", cls.secondary_video_audio_codec),
            secondary_video_audio_bitrate=os.getenv("SECONDARY_VIDEO_AUDIO_BITRATE", cls.secondary_video_audio_bitrate), secondary_video_cpu_used=int(os.getenv("SECONDARY_VIDEO_CPU_USED", cls.secondary_video_cpu_used)),
            ffmpeg_timeout_seconds=int(os.getenv("FFMPEG_TIMEOUT_SECONDS", cls.ffmpeg_timeout_seconds)), local_retain_sources=os.getenv("LOCAL_RETAIN_SOURCES", "true").lower() == "true",
            local_input_min_age_seconds=int(os.getenv("LOCAL_INPUT_MIN_AGE_SECONDS", cls.local_input_min_age_seconds)), source_folder_id=os.getenv("GDRIVE_SOURCE_FOLDER_ID", ""),
            target_folder_id=os.getenv("GDRIVE_TARGET_FOLDER_ID", ""), archive_folder_id=os.getenv("GDRIVE_ARCHIVE_FOLDER_ID", ""), original_transcript_subdir=os.getenv("ORIGINAL_TRANSCRIPT_SUBDIR", cls.original_transcript_subdir),
            resume_enabled=os.getenv("RESUME_ENABLED", "true").lower() == "true", normalize_legacy_names=os.getenv("NORMALIZE_LEGACY_NAMES", "true").lower() == "true",
            rename_processed_duplicates=os.getenv("RENAME_PROCESSED_DUPLICATES", "true").lower() == "true", automatic_output_deduplication=os.getenv("AUTOMATIC_OUTPUT_DEDUPLICATION", "false").lower() == "true",
            max_parallel_videos=int(os.getenv("MAX_PARALLEL_VIDEOS", cls.max_parallel_videos)), duplicate_name_similarity_threshold=float(os.getenv("DUPLICATE_NAME_SIMILARITY_THRESHOLD", cls.duplicate_name_similarity_threshold)),
            duplicate_duration_tolerance_seconds=float(os.getenv("DUPLICATE_DURATION_TOLERANCE_SECONDS", cls.duplicate_duration_tolerance_seconds)), duplicate_visual_similarity_threshold=float(os.getenv("DUPLICATE_VISUAL_SIMILARITY_THRESHOLD", cls.duplicate_visual_similarity_threshold)),
            ffmpeg_avoid_reencode=os.getenv("FFMPEG_AVOID_REENCODE", "true").lower() == "true", tts_enabled=os.getenv("TTS_ENABLED", "false").lower() == "true",
            tts_required=os.getenv("TTS_REQUIRED", "false").lower() == "true", tts_provider=os.getenv("TTS_PROVIDER", cls.tts_provider), tts_voice=os.getenv("TTS_VOICE", cls.tts_voice),
            tts_model_path=Path(os.getenv("TTS_MODEL_PATH", cls.tts_model_path)), tts_voices_path=Path(os.getenv("TTS_VOICES_PATH", cls.tts_voices_path)),
            ...
            tts_sample_rate=int(os.getenv("TTS_SAMPLE_RATE", cls.tts_sample_rate)), tts_audio_bitrate=os.getenv("TTS_AUDIO_BITRATE", cls.tts_audio_bitrate), tts_webm_audio_bitrate=os.getenv("TTS_WEBM_AUDIO_BITRATE", cls.tts_webm_audio_bitrate),
            tts_generate_webm=os.getenv("TTS_GENERATE_WEBM", "true").lower() == "true", google_credentials_file=Path(os.getenv("GOOGLE_CREDENTIALS_FILE", cls.google_credentials_file)),
            google_token_file=Path(os.getenv("GOOGLE_TOKEN_FILE", cls.google_token_file)), google_profile=os.getenv("GOOGLE_PROFILE", cls.google_profile), rclone_config_file=Path(os.getenv("RCLONE_CONFIG_FILE", cls.rclone_config_file)),
            rclone_binary_file=Path(os.getenv("RCLONE_BINARY_FILE", cls.rclone_binary_file)), rclone_remote=os.getenv("RCLONE_REMOTE", cls.rclone_remote), provider_profile_dir=Path(os.getenv("PROVIDER_PROFILE_DIR", cls.provider_profile_dir)),
            run_lock_file=Path(os.getenv("RUN_LOCK_FILE", cls.run_lock_file)), auto_bootstrap_rclone=os.getenv("AUTO_BOOTSTRAP_RCLONE", "true").lower() == "true", auto_update_rclone=os.getenv("AUTO_UPDATE_RCLONE", "false").lower() == "true",
            auto_tune_resources=os.getenv("AUTO_TUNE_RESOURCES", "true").lower() == "true", resource_profile=os.getenv("RESOURCE_PROFILE", cls.resource_profile),
            detected_logical_cpus=int(os.getenv("DETECTED_LOGICAL_CPUS", cls.detected_logical_cpus)), detected_memory_gb=float(os.getenv("DETECTED_MEMORY_GB", cls.detected_memory_gb)),
        )
